...
# Created by GitHub user foobar167
# Modified 11/2019 by Brian Ward
...
class CanvasImage:
    ...
    def grid_forget(self, **kw):
        """ Remove CanvasImage from parent widget"""
        self.__imframe.grid_forget(**kw)
        self.destroy()
    ...
    def __show_image(self):
        ...
        if int(x2 - x1) > 0 and int(y2 - y1) > 0:  # show image if it in the visible area
            ...
            imageid = self.canvas.create_image(max(box_canvas[0], box_img_int[0]), max(box_canvas[1], box_img_int[1]), anchor='nw', image=imagetk)
    ...
    def __wheel(self, event):
        ...
        if OS != 'Darwin' and self.outside(x, y): return  # zoom only inside image area
        scale = 1.0
